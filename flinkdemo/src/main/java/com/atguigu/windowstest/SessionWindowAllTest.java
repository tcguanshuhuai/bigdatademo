...
public class SessionWindowAllTest {
  public static void main(String[] args) throws Exception {
    StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
    env.setParallelism(2);
      URL url = SessionWindowAllTest.class.getClassLoader().getResource("input/words2.txt");
      ...
      source.map(x -> {
        ...
        .windowAll(EventTimeSessionWindows.withGap(Duration.ofSeconds(3))).process(new MyProcessAllWindowFunction())
        .print();
    env.execute();
  }

    // 自定义处理窗口函数，输出当前的窗口信息和word count
    public static class MyProcessAllWindowFunction extends ProcessAllWindowFunction<Tuple2<String, Long>, String, TimeWindow> {
        @Override
        public void process(Context context, Iterable<Tuple2<String, Long>> elements, Collector<String> out) throws Exception {
            ...
            out.collect("左开右闭" + "窗口" + start + " ~ " + end + "中共有" + count + "个");
        }
